...
function TodoPage(){
  ...
  const [todoList, updateTodoList] = React.useState(savedTodoList);
  ...
  return(
    ...
    <Header> Navin's Todo </Header>
       ...
       <H2> Things to do </H2>
        ...
        {!incompleteTodoList.length && <div className="ml-9 mb-4"> No todos here!</div>}
      
  {incompleteTodoList.map(t => <TodoRow todo= {t} key={t.id} done={false} onStatusChange={markAsDone} onDelete={onTodoDelete}>  </TodoRow>)}
      ...
      {showTodoForm && <TodoForm onClose={hideTodoForm} onCreate={addTodo} />}
       ...
       {!completeTodoList.length && <div className="ml-9"> Nothing done yet!</div>}

      {completeTodoList.map(t=><TodoRow todo= {t} key={t.id} done={true} onStatusChange= {markAsNotDone} onDelete={onTodoDelete}> </TodoRow>)}
